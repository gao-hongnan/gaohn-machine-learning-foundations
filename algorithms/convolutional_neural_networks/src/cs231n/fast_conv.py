...
def im2col(
    array: np.array, filter_dim: Tuple[int, int] = (3, 3), pad: int = 0, stride: int = 1
) -> np.array:
    """
    ...
    h_f, w_f = filter_dim
    array_pad = np.pad(
        array=array, pad_width=((0, 0), (0, 0), (pad, pad), (pad, pad)), mode="constant"
    )
    k, i, j = get_im2col_idx(
        array_shape=array.shape, filter_dim=filter_dim, pad=pad, stride=stride
    )
...
def col2im(
    ...
    k, i, j = get_im2col_idx(
        array_shape=array_shape, filter_dim=filter_dim, pad=pad, stride=stride
    )
